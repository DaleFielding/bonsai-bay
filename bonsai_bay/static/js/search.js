...
/* Event listener to call the searchListings function
  ...
  fetch(`/search?query=${query}`)
    ...
    .then(data => {
      ...
      if (data.length === 0) {
        ...
        listingsContainer.innerHTML =
          `<p class="m-3">No results found for ${query}</p>`;
      } else {
        data.forEach(listing => {
          let listingDiv = document.createElement("div");
          listingDiv.classList
            .add("col-10", "col-sm-6", "col-md-4", "col-lg-3");
          listingDiv.innerHTML = `
            <div class="item-card-container">
              ...
              `<img src="data:image/jpeg;base64,${listing.image}" alt="${listing.title}
                    Image" style="max-width: 100%; height: auto" class="img-fluid">`
              :
